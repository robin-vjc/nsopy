...
def _one_norm_bregman_map(M, lambda_k, diff_d_k):
    # Bregman map, according to [1], Eq. 2.9, with f(x) := -d(lambda), and M*psi(x,y) := M/2*||lambda-lambda_k||_2
    # Old:
    # lambda_bregman = lambda_k + float(1.0)/M*diff_d_k
    # return self._project_on_dual_feasible_set(lambda_bregman)
    return lambda_k + float(1.0)/(M*len(lambda_k))*diff_d_k


class OneNormUPGM(DualMethod, Observable):
    """
    ...
    def __init__(self, oracle, projection_function, dimension=0, epsilon=UGM_DEFAULT_EPSILON, averaging=False):
        ...
        iteration).
        """
        super(OneNormUPGM, self).__init__()

        self.desc = '1-Norm UPGM, $\epsilon = {}$'.format(epsilon)
        self.oracle = oracle
        ...
        # for record keeping
        self.method_name = '1-Norm UPGM'
        self.parameter = epsilon

    def _bregman_map(self, M, lambda_k, subgrad_lambda_k):
        return self.projection_function(_one_norm_bregman_map(M, lambda_k, subgrad_lambda_k))
    ...
    def dual_step(self):
        ...
        while not smallest_i_k_found:
            ...
            if (-d_k_plus <= -self.d_hat_k
                             ...
                             + 2**(i_k-1)*self.L_k*(np.linalg.norm(lambda_k_plus-self.lambda_hat_k, 1)**2)
                             + 0.5*self.epsilon):
