...
import numpy as np
from nsopy.methods.method_loggers import GenericDualMethodLogger, DualDgmFgmMethodLogger
...
def test_averaged_UDGM_on_analytical_example():
    ...
    np.testing.assert_allclose(logger.lambda_k_iterates[-1], np.array([0.7, 0.9]), atol=0.1)
    # should be close to optimal value (-0.5) but is at -0.8
    np.testing.assert_allclose(logger.d_k_iterates[-1], -0.6, atol=0.1)
